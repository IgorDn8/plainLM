"""BDLRU model, substituting Attention with BDLRU"""
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.implementation == "orig":
        ...
        elif self.implementation == "hopscan_opt_nonorm":

            #b t n*h
            #A_qk = torch.softmax(gates,-1) # B T N H H+1
            nl = gates[:,:,:,:,-1]*v[:,:,:,:] # B T N H 
            A_qk = gates[:,:,:,:,:-1] # B T N H H
        ...
        elif self.implementation == "hopscan_opt_relu":

            #b t n*h
            A_qk = F.relu(gates) # B T N H H+1
        ...
        elif self.implementation == "hopscan_opt_silu":

            #b t n*h
            A_qk = F.silu(gates) # B T N H H+1
        ...
        elif self.implementation == "hopscan_opt_sigmoid":

            #b t n*h
            A_qk = F.sigmoid(gates) # B T N H H+1
            ...
            #b t n*h
            A_qk = gates/torch.sum(torch.abs(gates),-1,keepdim=True) # B T N H H+1
            nl = A_qk[:,:,:,:,-1]*v[:,:,:,:] # B T N H 
            A_qk = A_qk[:,:,:,:,:-1] # B T N H H
